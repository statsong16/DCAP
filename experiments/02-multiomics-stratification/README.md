...
| 항목 | 값 |
|---|---|
| 출처 | `multi_omics_set.RData` (저장소 루트) |
...
| **아래 결과의 데이터** | **합성** (실제 자료 수치는 이 저장소에 적지 않습니다 — CLAUDE.md §5, §7) |
...
- **위 수치는 전부 합성 자료입니다.** 실제 `multi_omics_set.RData` 로 돌린 결과는 이
  저장소에 적지 않습니다 (CLAUDE.md §7). `src/export_real.py --out data` 로 내보내면
  `run_all.sh` 의 나머지가 그대로 돌아가는 것까지는 확인했습니다.
- **§7 을 어디까지 적용했는지.** 위 "데이터" 표에는 실제 파일의 규격(행렬 크기, 결측률,
  값의 스케일)이 들어 있습니다. 전처리를 왜 그렇게 정했는지 적으려면 필요하고,
  개인을 식별할 수 없는 자료 형식 정보라고 판단했습니다. 반대로 코호트 구성
  (성별·enterotype 분포, BMI 요약값 같은 것)은 연구 결과에 해당하므로 적지 않았습니다.
  이 선을 다르게 긋고 싶으면 표의 마지막 네 행을 지우면 됩니다.
